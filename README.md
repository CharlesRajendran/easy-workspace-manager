...
[![Visual Studio Marketplace Version](https://img.shields.io/visual-studio-marketplace/v/charles-rajendran.vsc-workspace-command-manager?label=Marketplace&logo=visual-studio-code)](https://marketplace.visualstudio.com/items?itemName=charles-rajendran.vsc-workspace-command-manager)
...
**Workspace Command Manager** provides a central mission-control dashboard inside VS Code to:
...
* Automatically organize outputs into clean, dedicated project terminals (`[WCM] <project-name>`).

---

## Quick Usage

Let's see **Workspace Command Manager** in action:
...
2. **Open the Dashboard**:
   - Click the **Workspace Command Manager** icon in the **Activity Bar** (left sidebar), OR
   - Click **`$(folder-library) Workspace Command Manager`** in the **Status Bar** (bottom-left), OR
   - Press `F1` (or `Cmd+Shift+P` / `Ctrl+Shift+P`), type `Workspace Command Manager: Open Dashboard`, and press `Enter`.
...
- **Dedicated Project Terminals**: Creates clean terminals prefixed with `[WCM] <repo-name>` with working directories (`cwd`) set directly to the repository's path.
...
- **1-Click Terminal Cleanup**: Dedicated "Close Managed Terminals" action closes all active `[WCM]` terminals with a single click.
...
| **Close All Terminals** | `workspaceManager.stopAllTerminals` | Closes all `[WCM]` terminals managed by this extension. |

---

## Configuration & Settings

You can customize Workspace Command Manager through VS Code Settings (**Settings > Extensions > Workspace Command Manager**):
...
| `workspaceManager.terminalPrefix` | `string` | `"[WCM]"` | Prefix used for managed terminal names (e.g. `[WCM] web-frontend`). |
...
- Please read our [Contributing Guide](.github/CONTRIBUTING.md) for full development setup, coding standards, and testing instructions.
